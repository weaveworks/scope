...
func DemoReport(nodeCount int) report.Report {
	...
	for i := 0; i < connectionCount; i++ {
		...
		r.Host.NodeMetadatas["hostX"] = report.NodeMetadata{
			report.HostTimestampKey: time.Now().UTC().Format(time.RFC3339Nano),
			report.HostHostnameKey:  "host-x",
			report.HostLocalNetsKey: localNet.String(),
			report.HostOSKey:        "linux",
		}
	}
